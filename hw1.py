...
proID_sum = {}
for t in sales:
    cus, pro, item = t['Customer ID'],t['Product ID'],t['Item Cost']
...
with open(outFile, mode='w',newline='') as csv_file:
    fieldnames = ['Product ID', 'Costumer Count', 'Total Revenue']
    writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
    writer.writeheader()
    for ID in sort_ID:
        writer.writerow({'Product ID': ID, 'Costumer Count': proID_count[ID][1],'Total Revenue': proID_sum[ID]})
